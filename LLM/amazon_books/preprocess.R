...
# con_out <- file("Books_5_Cleaned.jsonl", "wb")
# stream_in(file("Books_5.json"), handler = function(df) {
#   df |>
...
# }, pagesize = 10000)
# close(con_out)
reviews <- stream_in(file("Books_5_Cleaned.jsonl"))

# con_out <- file("meta_Books_Cleaned.jsonl", "wb")
# stream_in(file("meta_Books.json"), handler = function(df) {
#   df |>
#     select(title, asin) |>
#     drop_na() |>
#     stream_out(con_out, pagesize = 5000, verbose = FALSE)
# }, pagesize = 10000)
# close(con_out)
metadata <- stream_in(file("meta_Books_Cleaned.jsonl"))

reviews2 <- reviews |>
  # Remove reviews with asins not appeared in the metadata
  filter(asin %in% metadata$asin) |>
  # Take mean overall for duplicated books reviewed by each user,
...
# reviews2 |> write_parquet("AmazonBooks.parquet", compression = "BROTLI")
# reviews2 <- read_parquet("AmazonBooks.parquet") |> as_tibble()
...
prompts <- reviews3 |>
  ...
  summarise(
    ...
    input = sprintf(
      '<x>User preference: "%s"\nUser Unpreference: "%s"\nWhether the user will like the target book "%s"?</x>',
...
stream_out(prompts2[1:64, ], file("amazon_books_gpt_train_64.jsonl"))
stream_out(prompts2[1:128, ], file("amazon_books_gpt_train_128.jsonl"))
stream_out(prompts2[1:1024, ], file("amazon_books_gpt_train_1024.jsonl"))
stream_out(prompts2[1025:2048, ], file("amazon_books_gpt_valid.jsonl"))
stream_out(prompts2[2049:3072, ], file("amazon_books_gpt_test.jsonl"))
