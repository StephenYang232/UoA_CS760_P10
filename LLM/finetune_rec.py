...
def train(
    ...
    resume_from_checkpoint: str = None,  # either training checkpoint or final adapter    prompt_template_name: str = "alpaca",  # The prompt template to use, will default to alpaca.
    token: str = ""
):
    if int(os.environ.get("LOCAL_RANK", 0)) == 0:
        print(
            ...
            f"use_flash_attention_2: {use_flash_attention_2}\n"
    ...
    tokenizer = LlamaTokenizer.from_pretrained(base_model, token=token or None)
    ...
    if sample > -1:
        train_data["train"] = train_data["train"].select(range(sample))
...
def generate_prompt(data_point, valid=False):
