...
# con_out <- file("Movies_and_TV_5.jsonl", "wb")
# stream_in(gzfile("Movies_and_TV_5.json.gz", "rb"), handler = function(df) {
#   df |>
...
# }, pagesize = 10000)
# close(con_out)
reviews <- stream_in(file("Movies_and_TV_5.jsonl"))

# con_out <- file("meta_Movies_and_TV.jsonl", "wb")
# stream_in(gzfile("meta_Movies_and_TV.json.gz", "rb"), handler = function(df) {
#   df |>
#     select(title, asin) |>
#     drop_na() |>
#     stream_out(con_out, pagesize = 5000, verbose = FALSE)
# }, pagesize = 10000)
# close(con_out)
metadata <- stream_in(file("meta_Movies_and_TV.jsonl"))

reviews2 <- reviews |>
  # Remove reviews with asins not appeared in the metadata
  filter(asin %in% metadata$asin) |>
  # Take mean overall for duplicated movies and TVs reviewed by each user,
...
# reviews2 |> write_parquet("AmazonMoviesTV.parquet", compression = "BROTLI")
# reviews2 <- read_parquet("AmazonMoviesTV.parquet") |> as_tibble()
...
prompts <- reviews3 |>
  group_by(reviewerID) |>
  summarise(
    instruction = "Given the user's preference and unpreference, identify whether the user will like the target movie or TV by answering \"Yes.\" or \"No.\".",
    input = sprintf(
      '<x>User preference: "%s"\nUser Unpreference: "%s"\nWhether the user will like the target movie or TV "%s"?</x>',
...
stream_out(prompts2[1:64, ], file("amazon_movies_tv_gpt_train_64.jsonl"))
stream_out(prompts2[1:128, ], file("amazon_movies_tv_gpt_train_128.jsonl"))
stream_out(prompts2[1:1024, ], file("amazon_movies_tv_gpt_train_1024.jsonl"))
stream_out(prompts2[1025:2048, ], file("amazon_movies_tv_gpt_valid.jsonl"))
stream_out(prompts2[2049:3072, ], file("amazon_movies_tv_gpt_test.jsonl"))
